...
public class RequestParameterDeal {
	
	private static Logger logger = LoggerFactory.getLogger(RequestParameterDeal.class);
...
//	            }
	 ...
	 * @return String
	 */
	public static String requestParamsStr(HttpServletRequest request){
		
		/**
		 * 收集request parameter参数
		 */
		StringBuilder params = new StringBuilder("requestParameters:");
		Map requestParams = request.getParameterMap();
		for (Iterator iter = requestParams.keySet().iterator(); iter.hasNext();) {
			String name = (String) iter.next();
			String[] values = (String[]) requestParams.get(name);
			...
			params.append("&"+name+"="+valueStr);
		}
		
		/**
		 * 收集request header参数
		 */
		params.append(" requestHeaders:");
