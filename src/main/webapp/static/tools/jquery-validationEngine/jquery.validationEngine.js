 ...
 (function($) {
	 ...
	 var methods = {
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "maxSize":
						...
						break;
					case "minByteSize":
                        errorMsg = methods._minByteSize(field, rules, i, options);
                        break;
                    case "maxByteSize":
                        errorMsg = methods._maxByteSize(field, rules, i, options);
                        break;
					case "min":
		...
		*/
		_handleStatusCssClasses: function(field, options) {
		 ...
		 _validityProp: {
			 ...
			 "minByteSize": "range-underflow",
			 "maxByteSize": "range-overflow",
			 "min": "range-underflow",
			 "max": "range-overflow",
			 "past": "type-mismatch",
		...
		*/
		...
		/**
         * Check the maximum byte size (in characters)
         *
         * @param {jqObject} field
         * @param {Array[String]} rules
         * @param {int} i rules index
         ...
         * @return an error string if validation failed
         */
        _maxByteSize: function(field, rules, i, options) {
            var max = rules[i + 1];
            var len = field.attr('value').replace(/[^\x00-\xff]/g, "**").length;
            if (len > max) {
                var rule = options.allrules.maxByteSize;
                return rule.alertText + max + rule.alertText2;
            }
        },
        /**
         * Check the minimum byte size (in characters)
         *
         * @param {jqObject} field
         * @param {Array[String]} rules
         * @param {int} i rules index
         ...
         * @return an error string if validation failed
         */
        _minByteSize: function(field, rules, i, options) {
            var min = rules[i + 1];
            var len = field.attr('value').replace(/[^\x00-\xff]/g, "**").length;
            if (len < min) {
                var rule = options.allrules.minByteSize;
                return rule.alertText + min + rule.alertText2;
            }
        },
		/**
