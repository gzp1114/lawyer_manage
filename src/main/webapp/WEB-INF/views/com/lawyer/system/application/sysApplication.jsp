...
<%@ include file="/static/common/default.jsp"%>
		...
		<div  class="form-group">
			<form  id="sysApplicationSearchForm" >
                   ...
                   <label class="col-sm-1 control-label no-padding-right" for="form-field-1"> 应用名称： </label> 
                   <input type="text" id="search_appName" name="search_appName" placeholder="应用名称" class="col-xs-10 col-sm-2"  value=""/> 
				&nbsp;&nbsp;&nbsp;&nbsp;
				<button class="btn btn-sm btn-success"  id="sysApplicationSearch">
...
<style type="text/css">
	...
	//查询按键操作事件
	$( "#sysApplicationSearch" ).on('click', function(e) {
		return search();
	});
	...
	function search(){
		$("#grid-table").jqGrid('setGridParam',{
        	postData:{"search_tenantId":$("#search_tenantId").val(),"search_appName":$("#search_appName").val()},
	...
	jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				...
				colNames:[
						...
						'操作'],
				colModel:[
					{name:'tenantId',width:50, sortable:false},
					{name:'appName',width:80, sortable:false},
					{name:'appCode',width:60, sortable:false},
					{name:'secretKey',width:80, sortable:false},
					{name:'appUrl',width:100, sortable:false},
	...
	function sysUserInfoModifyRow(id){
		...
		var dialog = $('<div></div>').dialog({
            ...
            cache:false,
			modal:true,
			title:"<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-pencil-square-o'></i> 修改应用基本信息</h4></div>",
			title_html:true,
			buttons:[ 
					{
			...
			create: function( event, ui ) {$(this).load("<c:url value='/sysApplication/toModify/"+id+"'/>");},
			close:function() {
                $(this).dialog("destroy");
            }
		});
	...
	$( "#sysUserInfoAddButton" ).on('click', function(e) {
		...
		var dialog = $('<div></div>').dialog({
            ...
            cache:false,
			modal:true,
			title:"<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-plus-square'></i> 应用信息新增</h4></div>",
			title_html:true,
			buttons:[ 
					{
						html: "<i class='ace-icon fa fa-check bigger-110'></i>&nbsp; 提交",
			...
			create: function( event, ui ) {$(this).load("<c:url value='/sysApplication/toAdd'/>");},
			close:function() {
                $(this).dialog("destroy");
            }
		});
