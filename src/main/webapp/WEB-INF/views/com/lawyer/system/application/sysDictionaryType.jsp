...
<%@ include file="/static/common/default.jsp"%>
		...
		<div  class="form-group">
			<form  id="sysDictionaryTypeSearchForm" >
                   ...
                   <label class="col-sm-1 control-label no-padding-right" for="form-field-1"> 类型名称： </label> 
                   <input type="text" id="search_typeName" name="search_typeName" placeholder="类型名称" class="col-xs-10 col-sm-2"  value=""/> 
				&nbsp;&nbsp;&nbsp;&nbsp;
				<button class="btn btn-sm btn-success"  id="sysDictionaryTypeSearch">
...
<style type="text/css">
	...
	jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				url: "${ctx}/sysDictionaryType/search",
				datatype: 'json',
				mtype: 'post',
				height: 'auto',
				width: 'auto',
				...
				colModel:[
					...
					{name:'refSysAppName',width:100, sortable:false},
	...
	function sysUserInfoModifyRow(id){
		...
		var dialog = $('<div></div>').dialog({
            ...
            cache:false,
			modal:true,
			title:"<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-pencil-square-o'></i> 修改字典类型信息</h4></div>",
			title_html:true,
			buttons:[ 
					{
			...
			create: function( event, ui ) {$(this).load("<c:url value='/sysDictionaryType/toModify/"+id+"'/>");},
			close:function() {
                $(this).dialog("destroy");
            }
		});
	...
	$( "#sysUserInfoAddButton" ).on('click', function(e) {
		...
		var dialog = $('<div></div>').dialog({
            ...
            cache:false,
			modal:true,
			title:"<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-plus-square'></i> 字典类型新增</h4></div>",
			title_html:true,
			buttons:[ 
					{
						html: "<i class='ace-icon fa fa-check bigger-110'></i>&nbsp; 提交",
			...
			create: function( event, ui ) {$(this).load("<c:url value='/sysDictionaryType/toAdd'/>");},
			close:function() {
                $(this).dialog("destroy");
            }
		});
	...
	//查询按键操作事件
	$( "#sysDictionaryTypeSearch" ).on('click', function(e) {
		return search();
	});
	...
	function search(){
		$("#grid-table").jqGrid('setGridParam',{
        	postData:{"search_tenantId":$("#search_tenantId").val(),"search_typeName":$("#search_typeName").val(),"search_index":$("#search_index").val()},
